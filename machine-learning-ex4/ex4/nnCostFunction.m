...
Theta2_grad = 1/m * Theta2_grad + lambda/(2*m) * Theta2;                       %正则化，修正梯度值  
Theta2_grad(:,1) = Theta2_grad(:,1) - lambda/(2*m) * Theta2(:,1);              %由于不惩罚偏执单元对应的列，所以把他减掉  
Theta1_grad = 1/m * Theta1_grad + lambda/(2*m) * Theta1;                       %同理修改Theta1_grad  
Theta1_grad(:,1) = Theta1_grad(:,1) - lambda/(2*m) * Theta1(:,1);  
